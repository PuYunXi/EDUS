...
namespace YUNXI.EDUS.Tests
{
    public abstract class EDUSTestBase : AbpIntegratedTestBase<EDUSTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected EDUSTestBase()
        {
        ...
        protected void UsingDbContext(Action<EDUSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EDUSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EDUSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EDUSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EDUSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EDUSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EDUSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EDUSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EDUSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EDUSDbContext, Task<T>> func)
        {
            T result;
